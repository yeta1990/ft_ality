        ...
        | `Key_down ->
          ...
          let current_state = update_state pressed_keys keyname grammar in 
            let (found, description) = get_description_by_key grammar current_state in  
              ...
              else if key_exists current_state grammar then (print_current_state current_state; let (_, description) = get_description_by_key grammar [keyname] in print_string description; print_string "\n\n")
